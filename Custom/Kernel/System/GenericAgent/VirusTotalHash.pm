...
package Kernel::System::GenericAgent::VirusTotalHash;
...
sub Run {
    ...
    my $NewValue2 = "<b>Suggested Threat Label</b> : $Data->{data}->{attributes}->{popular_threat_classification}->{suggested_threat_label}<br/>";
    ...
    my $NewValue3 = "<b>Last Analysis Result</b><br/>";
    $NewValue3 .= "<table class='DataTable'>
    <thead>
    <tr>
    <th>Engine</th>
    <th>Category</th>
    ...
    for my $NeededValue3 ( sort {lc $a cmp lc $b} keys %{ $Data->{data}->{attributes}->{last_analysis_results} } )
	{
        #skip over undetected / empty result.
        next if $Data->{data}->{attributes}->{last_analysis_results}->{$NeededValue3}->{category} eq 'undetected';
        
        #delete not needed data
        delete($Data->{data}->{attributes}->{last_analysis_results}->{$NeededValue3}->{engine_name});
        delete($Data->{data}->{attributes}->{last_analysis_results}->{$NeededValue3}->{engine_update});

        $NewValue3 .= "<tr><td>$NeededValue3</td>";
        for my $NeededSubValue ( sort {lc $a cmp lc $b} keys %{ $Data->{data}->{attributes}->{last_analysis_results}->{$NeededValue3} } )
	    {
            $NewValue3 .= "<td>$Data->{data}->{attributes}->{last_analysis_results}->{$NeededValue3}->{$NeededSubValue}</td>";
        }
        $NewValue3 .= "<tr/>";
    }
    $NewValue3 .= "</tbody></table>";
    ...
    my $NewArticleID = $ArticleBackendObject->ArticleCreate(
        ...
        UserID               => 1,                              
        From           => "system",
        Subject        => "TotalVirus Hash: $Data->{data}->{id}",               
        Body           => "$TableCSS This is response from TotalVirus for hash $Data->{data}->{id}<br/><br/>For more value, please see the attachment.<br/><br/>$NewValue1<br/>$NewValue2<br/>$NewValue3",
