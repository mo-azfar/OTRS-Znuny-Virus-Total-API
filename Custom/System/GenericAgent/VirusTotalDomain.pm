...
package Kernel::System::GenericAgent::VirusTotalDomain;
...
sub Run {
    ...
    my $NewArticleID = $ArticleBackendObject->ArticleCreate(
        ...
        UserID               => 1,                              
        From           => "system",
        Subject        => "TotalVirus Domain: $Data->{data}->{id}",               
        Body           => "$TableCSS This is response from TotalVirus for domain $Data->{data}->{id}<br/><br/>For more value, please see the attachment.<br/><br/>$NewValue1<br/>$NewValue2",
